...
const UpdateProgressModal: React.FC<UpdateProgressModalProps> = ({
  ...
  // 处理实际日期修改
  const handleActualDateChange = (id: string, date: dayjs.Dayjs | null) => {
    setProgressList(prevList => 
      prevList.map(item => 
        item.id === id 
          ? { ...item, actualDate: date ? date.format('YYYY-MM-DD') : '' }
          : item
      )
    );
  };
    ...
    },
    {
      ...
      render: (text: string, record: ProgressItem) => (
        <DatePicker
          ...
          onChange={(date) => handleActualDateChange(record.id, date)}
          format="YYYY-MM-DD"
          size="small"
          placeholder="选择日期"
          style={{ width: '100%' }}
