    ...
    try {
      ...
      progressData.forEach((item, originalIndex) => {
        ...
        if (values.hasOwnProperty(`${prefix}ActualWarehouseDate`)) {
          itemData.actual_storage_date = values[`${prefix}ActualWarehouseDate`]
      ...
      if (hasUpdates && order?.id) {
        // 为采购状态更新转换数据类型
        const purchaseUpdateData: (Partial<ProductionProgressData> & {
          id: number;
        })[] = batchUpdateData.map((item) => ({
          id: item.id!,
          order_date: item.order_date,
          expected_material_date: item.expected_material_date,
          actual_storage_date: item.actual_storage_date,
  ...
  return (
    ...
    >
      ...
      >
        <Card title="采购状态">
          {/* 厂内生产项 */}
          {progressData.filter((item) => item.item_type === "internal").length >
            0 && (
          ...
          {/* 外购项 */}
          {progressData.filter((item) => item.item_type === "external").length >
            0 && (
            <div className={styles.sectionContainer}>
                    ...
                    return (
                      <Col span={12} key={`external-${originalIndex}`}>
                        <div className={styles.itemCard}>
                          ...
                          {item.order_date && !editingOrderDate[`external-${originalIndex}`] ? (
                            <div className={styles.dateInfoRow}>
                              <label className={styles.dateLabel}>
                                下单日期:
                              </label>
